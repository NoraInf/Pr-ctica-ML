...
poder ir por cualquier lugar, en cualquier momento, sin que haya riesgo de
...
(Tabla 1) Antes de profundizar en el código, debemos hacer un análisis preliminar de nuestras
...
así como optimizar el gasto en seguridad o diseñar nuevas propuestas para mejorar el sistema actual.
...
(Tabla 2) Este análisis del resumen estadístico revela varias conclusiones clave sobre la distribución de las variables:
...
(Tabla 3) En la tabla de correlación simple con nuestra variable objetivo ViolentCrimesPerPop, observamos que la inmigración reciente (PctRecentImmig), el condado (county) y el porcentaje de oficiales (LemasPctOfficDrugUn) asignados a la unidad de drogas casi no tienen correlación alguna con los crímenes violentos.
...
(Tabla 6) Los intervalos de confianza confirman matemáticamente nuestras observaciones previas; los predictores PctKids2Par, NumStreet y PctUnemployed **no cruzan el cero**, demostrando *significancia estadística*. En cambio, los intervalos de los **divorcios** (TotalPctDiv) y la **presencia policial** (LemasSwFTPerPop) sí incluyen el cero, confirmando de forma definitiva la *falta de evidencia de que realmente influyan* en el crimen y, por tanto, en este modelo.
...
# Determinamos el número de efectos en la tabla 
...
# Extraemos los componentes del Error 
...
max_y <- max(hist(r, plot = FALSE)$density, dnorm(mean(r), mean(r), sd(r)))

hist(r, prob = TRUE, main = "Histograma de Residuos", xlab = "Residuos estudentizados", col = "lightgray", ylim = c(0, max_y * 1.05))
...
En el siguiente gráfico, dibujamos en el eje X el residuo de la observación anterior ($\epsilon_{i-1}$), y en el eje Y el residuo de la observación actual ($\epsilon_i$).
...
Planteamos el test de Durbin-Watson con la finalidad de determinar de forma clara y final la correlación de los datos. En consecuencia, tenemos el siguiente análisis:
  ...
  * **El Estádistico ($d$):** El test calcula la suma de las diferencias al cuadrado entre residuos consecutivos dividida por la varianza total de los residuos. La magia de esta fórmula es que siempre devuelve un valor acotado entre **0 y 4**. Se define de la siguiente forma:
...
(Tabla 9) El **Modelo AIC es estadísticamente superior** y nuestra opción definitiva. 
...
Tras la búsqueda de una mayor interpretabilidad para nuestro modelo, hemos reducido el número de variables a tres determinando que los factores del número de efectivos policiales y la tasa de divorcios es prescindible para nuestro modelo, nos quedamos con las siguientes variables:
...
(Tabla 10) Tras el diagnóstico del modelo AIC, observamos que el supuesto de homocedasticidad se cumplía por un *margen muy ajustado* ($p = 0.0537$). Para reforzar la validez de nuestras inferencias y corregir la ligera curvatura observada en los residuos, **aplicamos una transformación Box-Cox**.
...
```{r warning=FALSE, message=FALSE, echo=FALSE}
# Librerías necesarias para estética, modelado y maquetación
library(MASS)
library(ggplot2)
library(broom)
library(knitr)
...
# 6. Extraemos los coeficientes y métricas globales
tabla_resumen <- tidy(modelo_refinado_bc)
metricas_globales <- glance(modelo_refinado_bc)

# Preparamos el texto solo con el R2 Ajustado y el Estadístico F
texto_metricas <- sprintf(
  "R-cuadrado Ajustado: %.4f | Estadístico F: %.2f",
  metricas_globales$adj.r.squared, 
  metricas_globales$statistic
)

# Generamos la tabla bonita para el PDF
tabla_resumen %>%
  ...
  kable_styling(latex_options = c("striped", "hold_position"), 
                full_width = FALSE) %>%
  footnote(general = texto_metricas,
           general_title = "Métricas Globales del Modelo: ",
           footnote_as_chunk = TRUE)

# 7. Test de Breusch-Pagan formateado para el informe
bp_resultado <- bptest(modelo_refinado_bc)
cat(sprintf("Diagnóstico de Homocedasticidad (Test de Breusch-Pagan): BP = %.4f, p-valor = %.4f\n", 
            bp_resultado$statistic, bp_resultado$p.value))
```
